...
def apply_coupons(cart, coupons)
  ...
  coupons.each do |coupon|
    ...
    grocery_in_card = grocery_with_coupon
    count_to_apply = grocery_in_card && grocery_with_coupon[:count] >= coupon[:num]
    if grocery_in_card && count_to_apply
      cart << {item: "#{grocery_with_coupon[:item]} W/COUPON", price: coupon[:cost]/coupon[:num],
      clearence: grocery_with_coupon[:clearence], count: coupon[:num]} 
